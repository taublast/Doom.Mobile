...
// Modified 2025 by Nick Kovalsky aka AppoMobi
//
...
namespace ManagedDoom.Video
{
    public sealed class Renderer
    {
        ...
        private Patch pause;
        public int wipeBandWidth;
        public int wipeBandCount;
        private int wipeHeight;
        public byte[] ScreenSnapshot;
        ...
        public Renderer(Config config, GameContent content)
        {
            ...
            {
                //todo not working as intended cuz hardcoded 640x400 everywhere
            ...
            ScreenSnapshot = new byte[Screen.Data.Length];
        ...
        public void RenderWipe(Doom doom, byte[] destination)
        {
            ...
            for (var i = 0; i < wipeBandCount - 1; i++)
            {
                ...
                for (var x = x1; x < x2; x++)
                {
                    ...
                    if (copyLength > 0)
                    {
                        ...
                        Array.Copy(ScreenSnapshot, srcPos, Screen.Data, dstPos, copyLength);
                    }
                }
            }
        ...
        {
            Array.Copy(Screen.Data, ScreenSnapshot, Screen.Data.Length);
        }

        public void WriteData(uint[] colors, byte[] destination)
        {
        ...
        //+5fps 
        private unsafe void WriteDataUnsafe(uint[] colors, byte[] destination)
        {
            var screenData = Screen.Data;

            fixed (byte* screenPtrFixed = screenData)
            fixed (uint* colorsPtrFixed = colors)
            fixed (byte* destPtrFixed = destination)
            {
                uint* destUintPtr = (uint*)destPtrFixed;
                byte* screenPtr = screenPtrFixed;
                uint* colorsPtr = colorsPtrFixed;
                int length = destination.Length / 4;

                Parallel.For(0, length, i =>
                {
                    destUintPtr[i] = colorsPtr[screenPtr[i]];
                });
            }
        }

        public static int GetPaletteNumber(Player player)
        {
